...
class Live:
    ...
    def __call__(self, skip = 0):
        ...
        csv_str = 'Date,Time,Frame #,Brand Recognition,Matched File\n'
        while True:
            ...
            if skip_count < skip:
                ...
                csv_str += f'{str(td.day).zfill(2)}-{str(td.month).zfill(2)}-{td.year},{str(td.hour).zfill(2)}:{str(td.minute).zfill(2)}:{str(td.second).zfill(2)},{count},{res[0][0]},{res[1][0]}\n'
            ...
            ocv.imshow('Brand Recognition : Livestream', cv_dat)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description = 'Brand Recognition Live Inference Client.')
