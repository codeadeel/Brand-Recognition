...
"""
BRAND RECOGNITION LIVE INFERENCE CLIENT IN WEB BROWSER
======================================================
...
class Live:
    def __init__(self, addr, server_ip, tzone='Asia/Karachi', ytdl='/resources/youtube-dl', ffmpeg='/resources/ffmpeg', skip=0):
        """
        This method is used to initialize Brand Recognition live inference client
        ...
        self.csv_str = 'Date,Time,Frame #,Brand Recognition,Matched Files\n'
        self.stream_inference = br_client(self.sc_server_ip)
    ...
    def __str__(self):
        ...
        print(f'Brand Recognition Server IP Address: {self.sc_server_ip}')
    ...
    def __call__(self):
        ...
        while True:
            ...
            if self.skip_count < self.skip:
                ...
                self.csv_str += f'{str(td.day).zfill(2)}-{str(td.month).zfill(2)}-{td.year},{str(td.hour).zfill(2)}:{str(td.minute).zfill(2)}:{str(td.second).zfill(2)},{self.count},{self.res[0][0]},{self.res[1][0]}\n'
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description = 'Brand Recognition Browser Based Live Inference Client.')
